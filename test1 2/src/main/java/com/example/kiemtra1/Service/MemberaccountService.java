...
public class MemberaccountService {
    ...
    public MemberAccount updateAccount(String token, MemberAccount newAccount) {
        ...
        return memberAccountRepo.findById(memberData.getId()).map(account -> {
    ...
    public Boolean updatePassword(String token, MemberAccount newAccount) {
        ...
        if (passwordEncoder.matches(newAccount.getPassword(), ps)) {
            ...
            return false;
        } else return memberAccountRepo.findById(memberData.getId()).map(account -> {
    ...
    public String decodeQr(byte[] data) throws IOException, NotFoundException {
        Result result = new MultiFormatReader().decode(new BinaryBitmap(new HybridBinarizer(new BufferedImageLuminanceSource(ImageIO.read(new ByteArrayInputStream(data))))));
